...
- For an outsized character, describe **build, not scale**: "heavyweight-wrestler mass, human proportions at giant size" — never *colossal / enormous / monumental / towering*. Those words name "more" rather than a size, and the video model downstream keeps supplying more until the character has inflated out of the scene. Carry the same wording into the manifest's critical-details field, because that is the text prompt-writers copy. See **asset-scale** for the full scale cage (contacts + ceiling + banned vocabulary).
