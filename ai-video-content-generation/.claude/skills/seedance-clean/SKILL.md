...
AUDIO
[Only the needed diegetic sound / line — environmental and action SFX plus any spoken line, nothing else. Never a musical score, songs, or lyrics; never on-screen text, captions, subtitles, titles, or lower-thirds. Seedance adds an unprompted score if allowed, so lock the exclusion in positive form: "diegetic sound only, no music, no on-screen text." Music beds and titles are layered in the edit, where they stay controllable.]
...
- **Relative height & mass via body landmarks.** For any two characters sharing the frame, state where their bodies *meet*, not a ratio or centimetres (perspective wrecks ratios; models ignore cm). ❌ "David is much shorter", "Goliath is 1.8× taller" → ✅ "David's crown reaches Goliath's navel; Goliath is twice as broad across the shoulders." Copy the exact landmark line from the manifest's Scale field / the asset-scale Scale Bible. Also state **mass** (shoulder width), or a tall figure reads as a stretched normal one. Anchor multiples to a figure **in the frame, by tag** — ✅ "twice `@shield_bearer`'s shoulder width", ❌ "twice a man's shoulder width" (no on-screen referent). For a lone giant, use whatever *is* in frame as the yardstick — nearby crowd figures, or their own oversized prop; failing that, human-multiples: ❌ "huge", "three meters tall" → ✅ "stands as tall as two men on each other's shoulders."
- **A landmark is a floor — always write the ceiling too (the scale cage).** One landmark line does **not** hold across a generation: it fixes the minimum gap and leaves the maximum open, so the large figure drifts upward mid-clip. Field-tested: with only "the bearer's crown reaches just above the giant's waist," three different video models held it ~2s and then inflated the giant into a ~15 m titan whose calves the bearer no longer reached. Cage it instead:
  - **Two or three contacts, not one** — crown-to-belt, **kneecap-to-hip**, hanging-elbow-to-helmet. One contact can be satisfied by an inflating figure; three contradicting ones cannot. Include a contact that directly forbids the failure you've actually seen.
  - **An explicit ceiling in body terms** — "a little under twice his height", "two men of his height one on the other's shoulders would overtop him", "both men whole in one frame on one visible ground line", "the same size in the last frame as in the first."
  - **No unbounded scale words** — *colossal, enormous, monumental scale, gigantic, towering, earth-shaking, ground tremor* name "more", not a size, and the model keeps supplying more. Describe **build** ("heavyweight-wrestler mass, human proportions") and make weight **relative** ("steps heavier and slower than the other man's; the ground stays firm, the camera holds steady"). Seismic PHYSICS/AUDIO cues are read as evidence the figure must be bigger, and it gets resized to justify them — check STYLE for the same leak ("monumental scale").
  - **Restate the cage three times** — the full list in FIRST FRAME / BLOCKING, again in the ACTION beat that reveals it, compressed in POSITIVE LOCKS. Once is not enough; this is the documented exception to "say each important thing once."
- **Upload the scale-reference lineup** (e.g. `@cast_scale`) alongside the identity sheets in a multi-character shot, and keep the compared characters at **similar camera distance on a longer lens (narrower FOV)** so perspective doesn't inflate the nearer one — unless you deliberately want a low, wide shot to exaggerate the giant.
- **No god's-eye overhead in a scale shot.** An overhead discards the shared ground line the whole comparison rests on, and the model re-guesses both figures' sizes on the new angle — a confirmed contributor to mid-shot scale runaway. Use a **high three-quarter** that keeps both pairs of feet in frame: same "look how small he is" gag, evidence still in shot. If the shot must change angle on a scale reveal, restate every contact for the new angle and add "these contacts hold from the new angle."
...
- **Clean-plate audio, titles in post.** Generate only diegetic sound — environmental and action SFX plus any spoken line. Never generate a musical score, songs, or lyrics, and never generate on-screen text, captions, subtitles, titles, or lower-thirds. Seedance injects a score unprompted, so state the exclusion as a positive lock (`"diegetic sound only, no music, no on-screen text"`). Music and titles are layered in the edit, where they stay controllable and removable.
...
- Size-critical figure caged, not just landmarked — 2–3 contacts + an explicit **ceiling** + no unbounded scale words (colossal/monumental/towering/earth-shaking), restated in blocking, in the reveal beat, and in the locks?
- Multiples anchored to a figure that is actually in the frame, by tag?
- Weight relative rather than seismic — and no god's-eye overhead in a scale shot?
- Emotion through muscle movement, not labels?
- Audio diegetic only — environmental/action SFX + needed lines, with music and on-screen text/subtitles locked out in positive form?
...
- **Diegetic sound only** — environmental/action SFX and needed lines; never generate music, songs, on-screen text, captions, or subtitles (those are added in the edit).
